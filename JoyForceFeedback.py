...
def test_constant_ff(device_path='/dev/input/event0'):
    """
    Test sending a constant force-feedback effect to a joystick.
    """
    from evdev import ecodes
    ...
    # Create a constant effect using the defined structures
    effect = Effect()
    effect.type = ecodes.FF_CONSTANT
    effect.id = -1  # -1 for new effect
    effect.direction = 0  # 0 = north (depends on device)
    effect.ff_trigger.button = 0
    effect.ff_trigger.interval = 0
    effect.ff_replay.length = 2000  # 2 seconds
    effect.ff_replay.delay = 0
    effect.u.ff_constant_effect.level = 0x7fff  # max strength
    effect.u.ff_constant_effect.ff_envelope.attack_length = 0
    effect.u.ff_constant_effect.ff_envelope.attack_level = 0
    effect.u.ff_constant_effect.ff_envelope.fade_length = 0
    effect.u.ff_constant_effect.ff_envelope.fade_level = 0

    # Upload the effect to the device
    EVIOCSFF = 0x40304580  # ioctl code for uploading FF effect
    buf = ctypes.create_string_buffer(ctypes.sizeof(Effect))
    ctypes.memmove(buf, ctypes.addressof(effect), ctypes.sizeof(Effect))
    fcntl.ioctl(dev.fd, EVIOCSFF, buf)

    # Play the effect
    dev.write(ecodes.EV_FF, 0, 1)  # 0 = effect id, 1 = play

    print("Constant force-feedback effect sent to device.")

# Example usage:
test_constant_ff('/dev/input/event22')  # Replace X with your joystick event number
